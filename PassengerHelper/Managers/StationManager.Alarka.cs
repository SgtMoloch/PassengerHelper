...
public partial class StationManager
{
    private bool RunAlarkaJctTransferStationProcedure(PassengerLocomotiveSettings pls, HashSet<string> expectedSelectedDestinations, StationProcedureContext ctx, DirectionOfTravel directionOfTravel)
    {
        ...
        if (jctIsWestTerminus)
        {
            ...
            Loader.LogVerbose($"Train has alarkajct as the west terminus station");
            // use normal logic
            return true;
        }
        ...
        {
            Loader.LogVerbose($"Train has alarkajct as the east terminus station");
            if (alarkaIsWestTerminus)
            {
                // train is running jct <-> cochran <-> alarka; branch only
                Loader.LogVerbose($"Train has alarka as the west terminus station; Train is doing the alarka branch only");

                if (directionOfTravel == DirectionOfTravel.EAST)
                {
                    Loader.LogVerbose($"Train is heading East so selecting all pickup stations");
                ...
                {
                    Loader.LogVerbose($"Train is heading West, so selecting only cochran and alarka as normal");
                }
            ...
            if (directionOfTravel == DirectionOfTravel.EAST)
            {
                ...
                Loader.LogVerbose($"Train is now heading East, so selecting alarka and cochran as pickup stations if needed");
                if (hasAlarkaPickup)
                {
                    Loader.LogVerbose($"adding alarka");
                    expectedSelectedDestinations.Add(StationIds.Alarka);
                }

                if (hasCochranPickup)
                {
                    Loader.LogVerbose($"adding cochran");
        ...
        Loader.LogVerbose($"Train has a station other than alarkajct for the east terminus");
        ...
        if (alarkaIsWestTerminus)
        {
            ...
            if (directionOfTravel == DirectionOfTravel.EAST)
            {
                ...
                Loader.LogVerbose($"Train is going east and is at or past alarkajct, using normal logic");
                return true;
            }
            ...
            if (directionOfTravel == DirectionOfTravel.WEST)
            {
                ...
                if (currentIndex_Pickup < alarkaJctIndex_Pickup)
                {
                    ...
                    Loader.LogVerbose($"Train is heading West, and is before jct, use normal logic");
        ...
        Loader.LogVerbose($"Train is long distance train with no alarka branch. Checking if alarka and cochran are pickup stations");

        bool addAlarkaAndCochran = true;
        bool pickUpContainsAlarkaAndCochran = hasAlarkaPickup && hasCochranPickup;
        Loader.LogVerbose($"Alarka and cochran are pickup stations: {pickUpContainsAlarkaAndCochran}");
        ...
        if (pickUpContainsAlarkaAndCochran)
        {
            ...
            Loader.LogVerbose($"Checking if train is before Alarka jct, based on current direction of travel");
            ...
            Loader.LogVerbose($"train is heading west towards AlarkaJct: {dotWestAndBeforeAlarkaJct}, train is heading east towards AlarkaJct: {dotEastAndBeforeAlarkaJct}");

            Loader.LogVerbose($"Ensuring that cochran and alarka are not stop at stations");
            bool stopDoesNotIncludeAlarkaAndCochran = !hasAlarkaStopAt && !hasCochranStopAt;
            Loader.LogVerbose($"cochran and alarka are not stop at stations: {stopDoesNotIncludeAlarkaAndCochran}");
